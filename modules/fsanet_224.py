...
class ConvModule(nn.Module):
    def __init__(self, in_channels, out_channels, activation='relu', ksize=5, stride = 2, 
    padding = 2):
        super(ConvModule, self).__init__()

        self.conv = nn.Conv2d(in_channels, out_channels, ksize, stride, padding)
        self.bn = nn.BatchNorm2d(out_channels)
...
class MultiStreamMultiStage(nn.Module):
    def __init__(self,in_channels):
        ...
        self.s0_conv = ConvModule(16,32,'relu')

        self.s0_conv1_0 = SepConvBlock(32,32,'relu')
        self.s0_conv1_1 = SepConvBlock(32,64,'relu')

        self.s0_conv1_out = Conv2dAct(64,128,1,'relu')

        self.s0_conv2_0 = SepConvBlock(64,128,'relu')
        self.s0_conv2_1 = SepConvBlock(128,128,'relu')
        self.s0_conv2_out = Conv2dAct(128,128,1,'relu')

        self.s0_conv3_0 = SepConvBlock(128,256,'relu')
        self.s0_conv3_1 = SepConvBlock(256,256,'relu')
        self.s0_conv3_out = Conv2dAct(256,128,1,'relu')

        # Stream 1 Layers #
        self.maxpool = nn.MaxPool2d(2)
        self.s1_conv0 = SepConvBlock(in_channels,16,'relu')

        self.s1_conv = ConvModule(16,32,'tanh')

        self.s1_conv1_0 = SepConvBlock(32,32,'tanh')
        self.s1_conv1_1 = SepConvBlock(32,64,'tanh')
        self.s1_conv1_out = Conv2dAct(64,128,1,'tanh')

        self.s1_conv2_0 = SepConvBlock(64,128,'tanh')
        self.s1_conv2_1 = SepConvBlock(128,128,'tanh')
        self.s1_conv2_out = Conv2dAct(128,128,1,'tanh')

        self.s1_conv3_0 = SepConvBlock(128,256,'tanh')
        self.s1_conv3_1 = SepConvBlock(256,256,'tanh')
        self.s1_conv3_out = Conv2dAct(256,128,1,'tanh')
    ...
    def forward(self,x):
        ...
        s0_x = self.s0_conv(s0_x)

        s1_x = self.s1_conv0(x)
        s1_x = self.maxpool(s1_x)
        s1_x = self.s1_conv(s1_x)
...
class ExtractSSRParams(nn.Module):
    def __init__(self,bins,classes = 2):
...
class FSANet(nn.Module):
    def __init__(self,var=False):
        ...
        self.fgsm = FineGrainedStructureMapping(128,num_primcaps,mdim,var) #channels: feature maps
        ...
        self.esp_s1 = ExtractSSRParams(3,2)
        self.esp_s2 = ExtractSSRParams(3,2)
        self.esp_s3 = ExtractSSRParams(3,2)
        self.ssr = SSRLayer(3)
